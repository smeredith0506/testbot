...
module Testbot::Runner
  ...
  class Runner
    ...
    def before_run(job)
      ...
      bundler_cmd = (RubyEnv.bundler?(job.project) ? [rvm_prefix, "bundle install --without development deployment --no-prune || bundle &&", rvm_prefix, "bundle exec"] : [rvm_prefix]).compact.join(" ")
